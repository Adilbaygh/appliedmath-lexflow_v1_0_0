...
- the Theorem 1 proof in `THEOREMS_AND_PROOFS.md` now carries the optimal-set
  ...
  nonterminal-user remark, and Remark 2 states when Stage 3 is redundant;
...
- `scenario.py` writes Table A2 (period-level parameters of the controlled
  scenario) and the reach-capacity ratios behind Appendix A.4
  (`table_A2_controlled_scenario_*`), so that every numerical table of the
  manuscript is regenerated by `run_analysis.py`; the README lists the source
  file of every manuscript table and figure.
